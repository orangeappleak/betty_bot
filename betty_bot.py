...
async def help(ctx):
    try:
        ...
        help_embed.add_field(name="register->",value="register the author's name into the database")
        help_embed.add_field(name="(admin_command) create_channel [name][category][type]->",value="create a text or a voice channel based on the given parameters")
        ...
        help_embed.add_field(name="(admin_command) kick [member]->",value="kisks the specified user from the server")
...
async def profile(ctx,member:discord.Member):
    try:
        ...
        for role in member.roles:
            r.append(role.name)
        iembed=discord.Embed(title="Info for the user:{}".format(member.name),color=discord.Colour.red())
